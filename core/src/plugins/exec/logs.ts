...
export class ExecLogsFollower {
  ...
  private async followLogs(opts: LogOpts) {
    ...
    this.events.on("streamEnd", async () => {
    ...
    this.events.on("error", async (errorMsg: string) => {
      this.log.debug(
        `<Streaming log from local process for service ${this.serviceName} failed with error: ${errorMsg}>`
      )
      this.unwatch()
    })
  ...
  private async startWatch(opts: LogOpts) {
    ...
    try {
      ...
      this.events.emit("error", `Starting file watcher failed with error ${err.message}`)
      return
    }
    ...
    try {
      ...
      return new Promise((res, rej) => {
        splitStream.on("end", () => {
          ...
          if (follow) {
            this.events.emit("streamEnd")
          }
          res({})
        })
        splitStream.on("error", (err) => {
          this.events.emit("error", `Reading stream failed with error: ${err.message}`)
          rej({})
          return
        })
      })
    } catch (err) {
      this.events.emit("error", `Tailing file failed with error: ${err.message}`)
      return
    }
  }
  ...
  private async getLineCount(): Promise<number> {
    try {
      ...
      return new Promise((res, _rej) => {
        ...
        splitStream.on("error", (err) => {
          this.events.emit("error", `Streaming line count failed with error: ${err.message}`)
        })
        splitStream.on("end", () => res(count))
      })
    } catch (err) {
      this.events.emit("error", `Counting lines failed with error: ${err.message}`)
      return 0
    }
  }
}
